...
const handleCheckout = async () => {
 ...
 if (cart.length === 0) {
  ...
  return ( 
  <div className="min-h-screen bg-slate-50/50 py-4 md:py-12">
          ...
          return Object.entries(groupedCart).map(([productId, items]) => (
            <div key={productId} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
              ...
              {items.map((item) => {
                ...
                return (
                <div key={item.variantId} className="flex flex-col gap-3 border-t border-slate-100 pt-4 pb-2">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 ">
                      <div>
                        <p className="text-xs font-bold text-slate-800">{item.variantName}</p>
                        {hasDiscount && (
                     ...
                     <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg h-8 px-1">
                        <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl h-9 px-2">
                          <button onClick={() => updateQuantity(item.variantId, item.quantity - 1)} className="p-1.5"><Minus size={12} /></button>
                          <span className="w-6 text-center text-xs font-black">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.variantId, item.quantity + 1)} className="p-1.5"><Plus size={12} /></button>
                        </div>
      ...
      <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 sticky top-24 space-y-6">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              {/* Inputs de datos del cliente */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Nombre Completo</label>
                <input 
                  type="text" 
                  placeholder="Ej: Juan Pérez" 
                  className="w-full bg-slate-50 rounded-xl px-4 py-3 text-sm outline-none border border-slate-200 focus:border-aquiles-primary"
                  value={nombreCliente} 
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 rounded-xl px-4 py-3 text-sm outline-none border border-slate-200"
                  value={emailCliente} 
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex bg-slate-50 rounded-xl border border-slate-200 overflow-hidden focus-within:border-aquiles-primary">
